...
class TestFileStorage_instantiation(unittest.TestCase):
    """unittests for testing instantiation FileStorage class."""
...
class TestFileStorage_methods(unittest.TestCase):
    ...
    def test_save(self):
        ...
        def test_reload_y(self):
            ...
            self.assertIn(f'BaseModel.{bm.id}', yobjs)
            self.assertIn(f'User.{us.id}', yobjs)
            self.assertIn(f'State.{st.id}', yobjs)
            self.assertIn(f'Place.{pl.id}', yobjs)
            self.assertIn(f'City.{cy.id}', yobjs)
            self.assertIn(f'Amenity.{am.id}', yobjs)
            self.assertIn(f'Review.{rv.id}', yobjs)
